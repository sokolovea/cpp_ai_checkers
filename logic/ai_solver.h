 ...
 */
    explicit DepthSearchSolver() = default;
private:
...
public:
     ...
     */
    static std::vector<Situation> solveDepthSearch(Situation start, Target target, size_t parMaxDepth) {
        std::vector<Situation> tempPath{start};
        std::vector<Situation> foundPath;
        depthSearchRecursive(start, target, parMaxDepth, tempPath, foundPath);
        return foundPath;
    }
